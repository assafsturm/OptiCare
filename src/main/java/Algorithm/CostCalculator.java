...
public class CostCalculator {
    ...
    private final UnassignedCostStrategy unassigned;
    ...
    public CostCalculator(RiskMatrix riskMatrix, AlgorithmConfig config, RoomTopologyGraph topologyGraph) {
        ...
        this.unassigned = new UnassignedCostStrategy(config);
        this.strategies = List.of(safety, clinical, policy, transfer, unassigned);
    }

    
    // Z = C_safety + C_clinical + C_policy + C_transfer + C_unassigned.

    // overload when no baseline exists C_transfer will be 0
    ...
    public double computeCUnassigned(AssignmentState state, Department department) {
        return unassigned.computeTotal(state, department, null, null);
    }
}
